...
**输入**：`/opsx:explore` 后的参数就是用户想探索的主题。比如：
- 模糊想法：“实时协作”
- 具体问题：“鉴权系统越来越难维护”
- 某个 change 名称：“add-dark-mode”（在对应变更上下文中探索）
- 方案比较：“这个场景下 postgres vs sqlite”
- 不带参数（直接进入探索）
...
如果用户提到了具体 change 名称，请读取对应工件作为上下文。
...
当结论逐渐成形时，你可以给一个总结，但不是必须。有时“思考过程本身”就是价值。
